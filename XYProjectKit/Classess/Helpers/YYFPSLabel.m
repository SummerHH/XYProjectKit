...
- (void)panGestureClick:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.x = (self.x - kScreenWidth * 0.5) > 0 ? (kScreenWidth - self.width - 20) : 20;
            self.y = self.y > 80 ? self.y : 80;
        }];
    }
}
...
- (void)tick:(CADisplayLink *)link {
    ...
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%ld FPS",(long)round(fps)]];
