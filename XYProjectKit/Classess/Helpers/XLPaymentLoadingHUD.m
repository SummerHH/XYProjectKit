...
#define BlueColor [UIColor colorWithRed:26/255.0 green:26/255.0 blue:26/255.0 alpha:1]
...
+(XLPaymentLoadingHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    XLPaymentLoadingHUD *hud = [[XLPaymentLoadingHUD alloc] initWithFrame:[UIScreen mainScreen].bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
-(void)start{
    _link.paused = false;
     [self performSelector:@selector(endLoading) withObject:nil afterDelay:13];
}
...
-(void)buildUI{
    ...
    _animationLayer.bounds = CGRectMake(0, 0, 45, 45);
