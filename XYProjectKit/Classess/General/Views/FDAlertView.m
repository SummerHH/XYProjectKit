...
- (void)initTitleAndIcon {
    ...
    if (_title != nil && ![_title isEqualToString:@""]) {
        ...
        if (_icon != nil) {
            _titleLabel.frame = CGRectMake(_iconImageView.frame.origin.x + _iconImageView.frame.size.width + SPACE_SMALL, 1, titleSize.width + 50, titleSize.height);
        }else {
            _titleLabel.frame = CGRectMake(0, 1, titleSize.width + 50, titleSize.height);
        }
    
        [_titleView addSubview:_titleLabel];
    }
    
    _titleView.frame = CGRectMake(0, MARGIN_TOP, _iconImageView.frame.size.width + SPACE_SMALL + titleSize.width + 50, MAX(_iconImageView.frame.size.height, titleSize.height));
...
- (void)show {
    ...
    /// 这里代码改了,下面注释的是源代码 打开源代码请注释这部分代码
...
- (void)setButtonTitleColor:(UIColor *)color font:(UIFont *)font atIndex:(NSInteger)index {
