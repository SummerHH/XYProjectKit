...
- (UIView *)alertView {
    if (!_alertView) {
//        CGFloat alertViewY = KDevice_Is_iphoneX ? (SCREEN_HEIGHT - kTopViewHeight - kDatePicHeight - kTabBarBottomHeight) : (SCREEN_HEIGHT - kTopViewHeight - kDatePicHeight);
        CGFloat alertViewHeight = isIPhoneNotchScreen() ? (kTopViewHeight + kDatePicHeight + kTabBarBottomHeight) : (kTopViewHeight + kDatePicHeight);
        
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT - kTopViewHeight - kDatePicHeight, SCREEN_WIDTH,alertViewHeight)];
